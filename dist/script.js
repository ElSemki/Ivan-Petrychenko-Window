...
window.addEventListener('DOMContentLoaded', function () {
  'use strict'; // Объект с данными выбора пользователя из калькулятора

  var modalState = {};
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_1__["default"])('.popup_engineer_btn', '.popup_engineer');
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_1__["default"])('.phone_link', '.popup');
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_1__["default"])('.popup_calc_btn', '.popup_calc');
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_1__["default"])('.popup_calc_button', '.popup_calc_profile', false);
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_1__["default"])('.popup_calc_profile_button', '.popup_calc_end', false);
  ...
  Object(_modules_timer__WEBPACK_IMPORTED_MODULE_5__["default"])('#timer', '2023-05-06'); // images();
});
...
/* harmony import */ var _getValidateInputs__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./getValidateInputs */ "./src/js/modules/getValidateInputs.js");
...
var changeModalState = function changeModalState(state) {
  ...
  // Выбор формы балкона
  var windowForm = document.querySelectorAll('.balcon_icons_img'); // Вписываем ширину

  var windowWidth = document.querySelectorAll('#width'); // Вписываем высоту

  var windowHeight = document.querySelectorAll('#height'); // Выбираем тип (select-список)

  var windowType = document.querySelectorAll('#view_type'); // Выбираем профиль (холодное или теплое (checkbox))

  var windowProfile = document.querySelectorAll('.checkbox'); // Валидация инпутов ввода ширины и высоты

  Object(_getValidateInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('#width');
  Object(_getValidateInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('#height'); // Функцтя для записи в объект выбора пользователя из калькулятора
        ...
        switch (item.nodeName) {
          ...
          case 'INPUT':
            ...
            if (item.getAttribute('type') === 'checkbox') {
              ...
              i === 0 ? state[prop] = 'Холодное' : state[prop] = 'Тёплое'; // По ТЗ можно выбрать только 1 чекбокс
              // Перебираем чекбоксы

              element.forEach(function (checkbox, j) {
  ...
  bindActionToElems(windowForm, 'click', 'form'); // Ширина окон

  bindActionToElems(windowWidth, 'input', 'width'); // Высота окон

  bindActionToElems(windowHeight, 'input', 'height'); // Тип

  bindActionToElems(windowType, 'change', 'type'); // Холодное или теплое
...
/* harmony import */ var _getValidateInputs__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./getValidateInputs */ "./src/js/modules/getValidateInputs.js");
...
function forms(formSelector, state) {
  ...
  Object(_getValidateInputs__WEBPACK_IMPORTED_MODULE_6__["default"])('input[name="user_phone"]');
  var message = {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', function (evt) {
      ...
      var formData = new FormData(form); // В случае с последней формой с калькулятора, мы должны отправить все данные с калькулятора (который вне формы, он не относится к форме) на сервер (эти данные, которые вне формы, находятся в объекте modalState), но в последней форме с калькулятора есть еще Имя и Телефон. Но это условие только в одной, последней форме! Проверим, если это та форма, у которой должен быть объект с данными
  ...
  function closeFormParent(parentFormSelector) {
    ...
    var modals = document.querySelectorAll(parentFormSelector); // Перебираем их

    modals.forEach(function (modal) {
      var modalStyles = window.getComputedStyle(modal); // Проверяем каждый родитель всплывающих форм на css классы
...
/***/ "./src/js/modules/getValidateInputs.js":
/*!*********************************************!*\
  !*** ./src/js/modules/getValidateInputs.js ***!
  \*********************************************/
/*! exports provided: default */
...
function getValidateInputs(inputSelector) {
  var phoneInputs = document.querySelectorAll(inputSelector);
...
/* harmony default export */ __webpack_exports__["default"] = (getValidateInputs);
...
function images() {
  ...
  var imgPopup = document.createElement('div'); // Находим контейнер с фотографиями

  var workSection = document.querySelector('.works'); // Большое изображение

  var bigImage = document.createElement('img'); // Добавим обертке для фото класс, как и у оберток форм

  imgPopup.classList.add('popup');
  workSection.append(imgPopup); // Выравниваем по центру экрана

  imgPopup.style.justifyContent = 'center'; // Выравниваем по вертикали

  imgPopup.style.alignItems = 'center'; // Скрываем элемент на странице

  imgPopup.style.display = 'none';
  imgPopup.append(bigImage); // Вешаем обработчик событий на контейнер с фото

  workSection.addEventListener('click', function (evt) {
    evt.preventDefault();
    var target = evt.target; // Если мы кликнули по элементу, который является фото (класс preview), то работаем с конкретной фотографией

    if (target && target.classList.contains('preview')) {
      // flex - Для выравнивания по центру
      imgPopup.style.display = 'flex'; // Изображение, на которое мы кликаем - находится в ссылке, которая ведет на такое же большое изображение
      ...
      var path = target.parentNode.getAttribute('href'); // Берем изображение, которое есть внутри модального окна
...
function modals(triggerSelector, modalSelector) {
  ...
  modal.addEventListener('click', function (evt) {
    if (evt.target === modal && closeClickOverlay || evt.target.textContent === '×') {
...
function tabs(tabsSelector, tabsContentSelector, tabsItemSelector, activeClass) {
  ...
  var tabContent = document.querySelectorAll(tabsContentSelector);
  var tabItems = document.querySelectorAll(tabsItemSelector);
  ...
  hideTabContent();
  showTabContent();
  tabsWrapper.addEventListener('click', function (evt) {
    var target = evt.target;

    if (target && target.classList.contains(tabsItemSelector.slice(1)) || target.parentNode.classList.contains(tabsItemSelector.slice(1))) {
